...
const EditPlaylistModal = ({ isOpen, onClose, playlist, onSuccess }) => {
  ...
  useEffect(() => {
    if (isOpen && playlist) {
      ...
      setCoverPreview(playlist.cover_image_url || null);
      setCoverImage(null);
      setError("");
    }
  }, [isOpen, playlist]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await updatePlaylist(playlist.playlist_id, playlistData);
              ...
              <div>
                ...
                {formData.isPublic && (
                  <p className="text-xs text-yellow-400 mt-2">
                    ⚠️ Public playlists can only contain public licks. If this playlist has private licks, you'll need to remove them first.
                  </p>
                )}
              </div>
